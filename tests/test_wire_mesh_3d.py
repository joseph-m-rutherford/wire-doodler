...
from doodler import Index, Real, WireMesh3D, r3, unify_meshes
...
def test_wire_mesh_3d_zero_h_raises():
    with pytest.raises(Unrecoverable):
        WireMesh3D(_named({'a': _POLY_A}), Real(0), _TOL)


def test_wire_mesh_3d_negative_h_raises():
    with pytest.raises(Unrecoverable):
        WireMesh3D(_named({'a': _POLY_A}), Real(-1), _TOL)


def test_wire_mesh_3d_zero_reltol_raises():
    with pytest.raises(Unrecoverable):
        WireMesh3D(_named({'a': _POLY_A}), _H, Real(0))
...
def test_wire_mesh_3d_description_parsed_as_radius():
    mesh = WireMesh3D({'a': ('r=2.5', _POLY_A)}, _H, _TOL)
...
def test_wire_mesh_3d_invalid_description_raises_unrecoverable():
    with pytest.raises(Unrecoverable, match='Invalid attribution at polyline-to-mesh: '):
        WireMesh3D({'a': ('0.5', _POLY_A)}, _H, _TOL)


def test_wire_mesh_3d_non_numeric_description_raises_unrecoverable():
    with pytest.raises(Unrecoverable, match='Invalid attribution at polyline-to-mesh: '):
        WireMesh3D({'a': ('r=not-a-number', _POLY_A)}, _H, _TOL)
...
def test_wire_mesh_3d_subsegment_radii_single_subsegment_tapers_both_ends():
    ...
    mesh = WireMesh3D({'a': ('r=5.0', poly)}, Real(1.0), _TOL)
...
def test_wire_mesh_3d_subsegment_radii_interior_joints_use_polyline_radius():
    ...
    mesh = WireMesh3D({'p': ('r=3.0', poly)}, Real(0.5), _TOL)
...
def test_wire_mesh_3d_subsegment_radii_independent_per_polyline():
    ...
    mesh = WireMesh3D({'a': ('r=2.0', poly_a), 'b': ('r=4.0', poly_b)}, Real(1.0), _TOL)
    assert mesh.subsegment_radii == [
...
def test_wire_mesh_3d_single_point_polyline_raises():
    with pytest.raises(Unrecoverable):
        WireMesh3D(_named({'a': [_pt(0, 0, 0)]}), _H, _TOL)
...
def test_wire_mesh_3d_intra_duplicate_endpoints_raises():
    ...
    with pytest.raises(Unrecoverable):
        WireMesh3D(_named({'loop': poly}), _H, _TOL)
...
def test_wire_mesh_3d_intra_nearby_interior_points_raises():
    ...
    with pytest.raises(Unrecoverable):
        WireMesh3D(_named({'close': poly}), _H, _TOL)
...
def test_wire_mesh_3d_inter_shared_start_start_supported():
    ...
    mesh = WireMesh3D(_named({'a': poly_a, 'b': poly_b}), _H, _TOL)
...
def test_wire_mesh_3d_inter_crossing_segments_raises():
    ...
    with pytest.raises(NotYetImplemented):
        WireMesh3D(_named({'a': poly_a, 'b': poly_b}), _H, _TOL)
...
def test_wire_mesh_3d_valid_construction():
    mesh = WireMesh3D(_named({'a': _POLY_A, 'b': _POLY_B}), _H, _TOL)
    assert set(mesh.named_polylines.keys()) == {'a', 'b'}
...
def test_wire_mesh_3d_subsegment_counts_exact_multiple():
    ...
    poly = [_pt(0, 0, 0), _pt(1, 0, 0), _pt(2, 0, 0)]
    mesh = WireMesh3D(_named({'p': poly}), _H, _TOL)
...
def test_wire_mesh_3d_subsegment_counts_non_multiple():
    ...
    poly = [_pt(0, 0, 0), _pt(1, 0, 0)]
    mesh = WireMesh3D(_named({'p': poly}), Real(0.3), _TOL)
    counts = mesh.named_subsegment_counts
    assert counts['p'] == [4]
...
def test_wire_mesh_3d_subsegment_counts_h_larger_than_segment():
    ...
    mesh = WireMesh3D(_named({'short': poly}), Real(1.0), _TOL)
    counts = mesh.named_subsegment_counts
...
def test_wire_mesh_3d_subsegment_counts_all_at_least_one():
    ...
    mesh = WireMesh3D(_named({'tiny': pts}), Real(1.0), _TOL)
...
def test_subsegment_index_single_polyline_single_segment():
    ...
    poly = [_pt(0, 0, 0), _pt(1, 0, 0)]
    mesh = WireMesh3D(_named({'a': poly}), Real(0.5), _TOL)
    idx = mesh.subsegment_index
    assert len(idx) == 2
    assert idx[0] == ('a', 0, 0, 0)
    assert idx[1] == ('a', 0, 1, 0)
...
def test_subsegment_index_single_polyline_two_segments():
    ...
    poly = [_pt(0, 0, 0), _pt(1, 0, 0), _pt(2, 0, 0)]
    mesh = WireMesh3D(_named({'p': poly}), Real(0.5), _TOL)
    idx = mesh.subsegment_index
    assert len(idx) == 4
    assert idx[0] == ('p', 0, 0, 0)
    assert idx[1] == ('p', 0, 1, 0)
    assert idx[2] == ('p', 1, 0, 0)
    assert idx[3] == ('p', 1, 1, 0)
...
def test_subsegment_index_two_polylines_lexicographic_order():
    ...
    mesh = WireMesh3D(_named({'b': poly_b, 'a': poly_a}), Real(1.0), _TOL)
    idx = mesh.subsegment_index
    # 'a' first: 1 subsegment; 'b' second: 2 subsegments -> 3 total
    assert len(idx) == 3
    assert idx[0] == ('a', 0, 0, 0)
    assert idx[1] == ('b', 0, 0, 1)
    assert idx[2] == ('b', 1, 0, 1)
...
def test_subsegment_index_immutable():
    mesh = WireMesh3D(_named({'a': _POLY_A}), _H, _TOL)
    with pytest.raises(NeverImplement):
...
def test_wire_mesh_3d_point_registry_exists():
    poly = [_pt(0, 0, 0), _pt(1, 0, 0)]
    mesh = WireMesh3D(_named({'a': poly}), Real(1.0), _TOL)
...
def test_wire_mesh_3d_point_pairs_match_endpoints():
    poly = [_pt(0, 0, 0), _pt(1, 0, 0), _pt(2, 0, 0)]
    mesh = WireMesh3D(_named({'p': poly}), Real(1.0), _TOL)
...
def test_wire_mesh_3d_inter_polyline_shared_point_same_index():
    ...
    mesh = WireMesh3D(_named({'a': poly_a, 'b': poly_b}), Real(1.0), _TOL)
    pairs = mesh.subsegment_point_pairs
...
def test_wire_mesh_3d_vertex_xyz_returns_copy():
    ...
    mesh = WireMesh3D(_named({'a': poly}), Real(10.0), _TOL)
...
def test_unify_meshes_no_shared_points():
    mesh_a = WireMesh3D(_named({'a': [_pt(0, 0, 0), _pt(1, 0, 0)]}), Real(1.0), _TOL)
    mesh_b = WireMesh3D(_named({'b': [_pt(5, 0, 0), _pt(6, 0, 0)]}), Real(1.0), _TOL)
...
def test_unify_meshes_with_shared_endpoint():
    mesh_a = WireMesh3D(_named({'a': [_pt(0, 0, 0), _pt(1, 0, 0)]}), Real(1.0), _TOL)
    mesh_b = WireMesh3D(_named({'b': [_pt(1, 0, 0), _pt(2, 0, 0)]}), Real(1.0), _TOL)
    ua, ub = unify_meshes(mesh_a, mesh_b)
...
def test_unify_meshes_shared_registry_identity():
    mesh_a = WireMesh3D(_named({'a': _POLY_A}), _H, _TOL)
    mesh_b = WireMesh3D(_named({'b': _POLY_B}), _H, _TOL)
    ua, ub = unify_meshes(mesh_a, mesh_b)
    assert ua.point_registry is ub.point_registry
...
def test_unify_meshes_preserves_subsegment_endpoints():
    mesh_a = WireMesh3D(_named({'a': [_pt(0, 0, 0), _pt(1, 0, 0)]}), Real(1.0), _TOL)
    mesh_b = WireMesh3D(_named({'b': [_pt(2, 0, 0), _pt(3, 0, 0)]}), Real(1.0), _TOL)
    ua, ub = unify_meshes(mesh_a, mesh_b)
...
def test_unify_meshes_preserves_mesh_functions():
    ...
    poly_a = [_pt(0, 0, 0), _pt(1, 0, 0)]
    poly_b = [_pt(1, 0, 0), _pt(2, 0, 0)]
    mesh_a = WireMesh3D(_named({'a': poly_a, 'b': poly_b}), Real(1.0), _TOL)
    mesh_b = WireMesh3D(_named({'c': [_pt(5, 0, 0), _pt(6, 0, 0)]}), Real(1.0), _TOL)
    ua, ub = unify_meshes(mesh_a, mesh_b)
